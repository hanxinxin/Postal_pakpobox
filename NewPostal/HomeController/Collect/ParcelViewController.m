...
@interface ParcelViewController ()<UITableViewDelegate,UITableViewDataSource>
...
//    [self getOrderList];
    ...
    [[AFNetWrokingAssistant shareAssistant] GETWithCompleteURL:[NSString stringWithFormat:@"%@%@",FuWuQiUrl,Get_BaoguoList] parameters:nil progress:^(id progress) {
        //        NSLog(@"请求成功 = %@",progress);
        ...
        for (int i=0; i<arr.count; i++) {
            ...
            ParcelMode * mode = [[ParcelMode alloc] init];
            mode.location = [dict objectForKey:@"lockerName"];
