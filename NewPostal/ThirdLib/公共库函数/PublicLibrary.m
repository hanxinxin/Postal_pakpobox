...
// 从故事版读取Controller
+ (nullable __kindof UIViewController *)controllerWithStoryboardName:(NSString *_Nullable)storyboardName identifier:(NSString *_Nullable)identifier{
...
// 读取本地图片
+ (nullable UIImage *)imageWithName:(NSString *_Nullable)name{
    UIImage *image = [UIImage imageNamed:name inBundle:[NSBundle bundleForClass:[self class]] compatibleWithTraitCollection:nil];
    return image;
}
+(UIImage *_Nullable)imageNamed:(NSString *_Nullable)imgName bundleNumber:(NSBundle * _Nullable )bundleName
{
    NSBundle *dynamicBundle = MR_Bundle;

    NSURL *bundelURL = [dynamicBundle URLForResource:@"PostalBundle" withExtension:@"bundle"];
...
+(UIImage *_Nullable)imageNamed:(NSString *_Nullable)imgName
{
    NSBundle *dynamicBundle = MR_Bundle;
