        ...
        switch (error.code) {
            case LAErrorPasscodeNotSet:{
               NSLog(@"认证无法启动，因为设备没有设置密码。");
                if ([self.delegate respondsToSelector:@selector(TouchVerify:)]) {
                    [self.delegate TouchVerify:5]; // 回调代理
                }
                break;
            }
...
//           你尚未设置Touch ID，请在手机系统“设置>Touch ID与密码“中添加指纹
            ...
            default:{
                NSLog(@"TouchID  不可用");
                if ([self.delegate respondsToSelector:@selector(TouchVerify:)]) {
                    [self.delegate TouchVerify:10]; // 回调代理
                }
                break;
            }
        }
...
//    LAPolicyDeviceOwnerAuthentication，指纹和数字密码的授权使用，当指纹可用且没有被锁定，授权后会进入指纹密码验证。不然的话会进入数字密码验证的页面。当系统数字密码没有设置不可用的时候，授权失败。如果数字密码输入不正确，连续6次输入数字密码都不正确后，会停用鉴定过一定的间隔后才能使用，间隔时间依次增长.
    self.context.localizedFallbackTitle = !lock?@"Please try again":@"Use the password";

    NSInteger lopli =  lock?LAPolicyDeviceOwnerAuthentication:LAPolicyDeviceOwnerAuthenticationWithBiometrics;
    NSString * result_cn = !lock?@"Please verify existing fingerprints":@"Too many errors. Password needed to enable TouchID";
    ...
    [contxt evaluatePolicy:lopli localizedReason:result_cn reply:^(BOOL success, NSError *error) {
        if (success) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                NSLog(@"验证成功");
                if ([self.delegate respondsToSelector:@selector(TouchVerify:)]) {
                    [self.delegate TouchVerify:1]; // 回调代理
                }
            });
        }else{
            switch (error.code) {
                case LAErrorSystemCancel:{
                    //系统取消授权，如其他APP切入
                    if ([self.delegate respondsToSelector:@selector(TouchVerify:)]) {
                        [self.delegate TouchVerify:0]; // 回调代理
                    }
                    break;
                }
