...
@interface ZDYQRCodeScanningVC () <SGQRCodeScanManagerDelegate, SGQRCodeAlbumManagerDelegate,CBCentralManagerDelegate>
{
    BOOL QRBool;
}
...
@implementation ZDYQRCodeScanningVC
...
//    //    [backBtn setTintColor:[UIColor blackColor]];
//        RightBtn.title = FGGetStringWithKeyFromTable(@"", @"Language");
//    [RightBtn setImage:[UIImage imageNamed:@"icon_scan_ad"]];
//        self.navigationItem.rightBarButtonItem = RightBtn;
    ...
    dispatch_after(delayTime, dispatch_get_main_queue(), ^{
        UIButton * back = [[UIButton alloc] init];
        back.frame = CGRectMake(15, 25, 28, 28);
        //    [back setImage:[UIImage imageNamed:@"popup_close"] forState:(UIControlStateNormal)];
...
//        [back setImageEdgeInsets:UIEdgeInsetsMake(0, 6, 6, 6)];
...
//            self.navigationController.navigationBar.subviews[0].alpha = 0.0;
        ...
        //    [back setImage:[UIImage imageNamed:@"popup_close"] forState:(UIControlStateNormal)];
        [rightB setImage:[UIImage imageNamed:@"icon_scan_ad-1"] forState:(UIControlStateNormal)];
        [rightB addTarget:self action:@selector(rightBTouch:) forControlEvents:(UIControlEventTouchUpInside)];
