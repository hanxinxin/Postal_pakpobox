...
@interface ScanCQRViewController () <SGQRCodeScanManagerDelegate, SGQRCodeAlbumManagerDelegate,CBCentralManagerDelegate>
{
    BOOL QRBool;
}
...
@implementation ScanCQRViewController
...
- (void)dealloc {
    NSLog(@"ScanCQRViewController - dealloc");
//    [self removeScanningView];
}
...
//    QR code, EAN 13, EAN 8, UPC-A, UPC-E, Code 128, Data Matrix, PDF417, Aztec, Interleaved 2 of 5, Code 39, Code 93, Codabar, DataBar
...
//                [self postParcel_take_app:@"240267b1-b657-4721-93da-5a1b12ea932d"];
                //                }
            }
            
        }else
        {
            NSLog(@"没有识别出取件的格式");///改为直接跳入post
            //            if ([self.delegate respondsToSelector:@selector(returnWCQRStr:)]) {
...
//            [self setupQRCodeScanning];
            [self pushLetterTBViewController:orderStr1];
        }
...
-(void)pushLetterTBViewController:(NSString *)str{
...
-(void)postParcel_take_app:(NSString*)IDStr
{
    ...
    NSLog(@"URL =%@  \n dict ====%@",[NSString stringWithFormat:@"%@%@",FuWuQiUrl,Post_parcel],dict);
...
//    [[AFNetWrokingAssistant shareAssistant] GETWithCompleteURL:[NSString stringWithFormat:@"%@%@%@",FuWuQiUrl,Get_take_info,@"240267b1-b657-4721-93da-5a1b12ea932d"] parameters:nil progress:^(id progress) {
